...
class ExperimentBuilder(object):
    def __init__(self, args, data):
        ...
        # #####################################
        # generator_layers = [32, 64, 64, 128, 128]
        # discriminator_layers = [32, 64, 64, 128, 128]
        #
        # gen_inner_layers = [gen_depth_per_layer, gen_depth_per_layer, gen_depth_per_layer, gen_depth_per_layer,
        #                     gen_depth_per_layer]
        # discr_inner_layers = [discr_depth_per_layer, discr_depth_per_layer, discr_depth_per_layer,
        #                       discr_depth_per_layer,
        #                       discr_depth_per_layer]
        # generator_layer_padding = ["SAME", "SAME", "SAME", "SAME", "SAME"]
        # #####################################
        ...
        self.tensorboard_update_interval = 5 #int(self.total_train_batches/100/self.num_gpus)
                ...
                for e in range(start_from_epoch, self.total_epochs):
                    ...
                    with tqdm.tqdm(total=self.total_train_batches) as pbar_train:
                        for iter in range(self.total_train_batches):
                            ...
                            for n in range(self.disc_iter):
                                ...
                                _, d_train_loss_value = sess.run(
                                    ...
                                    feed_dict={self.input_x_i: x_train_i,
                                               ...
                                               self.training_phase: True, self.random_rotate: False})
                            ...
                            pbar_train.update(1)
                    if e % (self.tensorboard_update_interval) == 0:
                        self.train_writer.add_summary(train_summaries, global_step=e)
                        self.validation_writer.add_summary(val_summaries, global_step=e)
