...
class DAGANDataset(object):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
        ...
        self.image_channel = self.x_train.shape[3]
        self.training_data_size = self.x_train.shape[0]
        self.validation_data_size = gen_batches * self.batch_size
        self.testing_data_size = self.x_test.shape[0]
    ...
    def get_batch(self, dataset_name):
        ...
        choose_samples = np.random.choice(self.datasets[dataset_name].shape[0], size=2 * self.batch_size,
                                          replace=True)
        ...
        for i in range(self.batch_size):
            x_input_batch_a.append(self.datasets[dataset_name][choose_samples_a[i]])
            x_input_batch_b.append(self.datasets[dataset_name][choose_samples_b[i]])
...
class DAGANImbalancedDataset(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
        ...
        self.training_data_size = self.x_train.shape[0] #np.sum([len(self.x_train[i]) for i in range(self.x_train.shape[0])])
        self.validation_data_size = self.x_val.shape[0] #np.sum([len(self.x_val[i]) for i in range(self.x_val.shape[0])])
        self.testing_data_size = self.x_test.shape[0] #np.sum([len(self.x_test[i]) for i in range(self.x_test.shape[0])])
        ...
        #     val_dict[idx] = {"sample_idx": j}
        #     idx += 1
        self.choose_gen_samples = np.random.choice([i for i in range(self.validation_data_size)],
        ...
        self.test_samples_per_label = gen_batches
        # self.choose_gen_labels = np.random.choice(self.x_val.shape[0], self.batch_size, replace=True)
        # self.choose_gen_samples = np.random.choice(len(self.x_val[0]), self.test_samples_per_label * self.batch_size,
        #                                            replace=True)
        # self.x_gen = self.x_val[self.choose_gen_labels]
        self.x_gen = self.x_val[self.choose_gen_samples]
...
class PedestrianDAGANDataset(DAGANDataset):
    def __init__(self, batch_size, last_training_class_index, reverse_channels, num_of_gpus, gen_batches):
