...
def get_occ_imgs(img, img_name,model,img_size, occ_size, occ_pixel, occ_stride, classes):
    # Get original image
    image = cv2.imread(img)
    image = cv2.resize(image, (img_size, img_size)).astype(np.float32)

    # Index of class with highest probability
    class_index = 1#np.argmax(classes)
...
def regularize(prob, norm, percentile, img_path, filename):
    ...
    f2.savefig('occ_exp/heatmap_reg_' + filename)
    plt.close('all')

    return reg_heat


def join(heat_reg, img, img_size, occ_size, filename):
    # Get original image
    ...
    cv2.imwrite('occ_exp/final_' + filename, inp_viz)
...
#     parser.add_argument('--img', type=str, default='D:/EffNet/crop_000016.png')  # Path of the input image
#     parser.add_argument('--weights_path', type=str, default='D:/EffNet/logs/Transfer_Learn_EfficientNetB0_epochs_350_lr_0.000020_batch_32_dropout_0/Model_EfficientNetB0_Person.h5')  # Path of the saved pre-trained model
